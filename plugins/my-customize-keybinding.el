...
(defun new-frame-do-some-initially-setting (&optional frame)
  ...
  (define-key key-translation-map [(control c) (control n)] [(control meta e)])
  (define-key key-translation-map [(control c) (control p)] [(control meta a)])
...
(require 'mark-lines)
(global-set-key [(meta d)] 'kill-word-or-delete-region)
(global-set-key [(meta K)] 'mark-lines-next-line) ;标记行 Alt-k
(global-set-key [(control c) (control K)] 'kill-current-line-and-indent)
(global-set-key [(control backspace)] 'backward-delete-to-line-beginning)
(global-set-key [(control ?\d)] 'backward-delete-to-line-beginning)
...
(global-set-key [(control b)] 'bury-buffer) ;将当前buffer放到最后 Ctrl-b
...
(global-set-key [(control h) (f1)] 'view-man-help) ;察看man帮助
...
;; 前一个相同的单词, 下一个相同的单词; 
(global-set-key [(meta F)] 'iy-go-to-char)
...
(add-hook 'html-mode-hook
          '(lambda ()
             (define-key html-mode-map [(control meta n)] 'sgml-skip-tag-forward)
             (define-key html-mode-map [(control meta p)] 'sgml-skip-tag-backward)
             ...
             (define-key html-mode-map [(control meta ?\s)] 'html-mark-sexp-tag)
             (define-key html-mode-map [(control meta k)] 'html-kill-sexp-tag)
             (define-key html-mode-map [(control c) (control V)] 'save-buffer-and-browse-url)
...
(add-hook 'nxml-mode-hook
          '(lambda ()
             (define-key nxml-mode-map [(control meta ?\s)] 'nxml-mark-sexp-tag)
             (define-key nxml-mode-map [(control meta k)] 'html-kill-sexp-tag)
             (setq sgml-xml-mode t)
             ))
...
(add-hook 'eshell-mode-hook
          '(lambda ()
             ...
             (define-key eshell-mode-map [(control meta n)] 'next-line)
             (define-key eshell-mode-map [(control meta p)] 'previous-line)
             (defalias 'f 'find-file)
             ))

;; shell-mode使用comint-mode-hook设置.
(add-hook 'shell-mode-hook
          '(lambda ()
             (define-key comint-mode-map [(meta \8)] 'keyboard-escape-quit)
             ))
...
(add-hook 'auto-complete-mode-hook
          '(lambda ()
             ...
             (define-key ac-complete-mode-map [(space)] 'ac-stop)
...
(add-hook 'isearch-mode-hook ;可以通过apropos命令,搜索`isearch-.*'来查找所有跟isearch有关的命令.
          '(lambda ()
             ...
             (define-key isearch-mode-map [(control meta y)] 'isearch-yank-secondary)
...
(setq ibuffer-saved-filter-groups
      '(("work"
         ("Ruby" (or (mode . ruby-mode)
                     (mode . rhtml-mode)
                     (mode . css-mode)))
         ("Gtalk" (or (mode . jabber-chat-mode)))
        ;; ("Help" (or (name . "\*Help\*")
        ;;             (name . "\*Apropos\*")
        ;;             (name . "\*info\*")))
        )))
...
(add-hook 'jabber-roster-mode-hook
          '(lambda ()
             (define-key jabber-roster-mode-map [(mouse-3)] 'jabber-roster-mouse-2-action-at-point)
             ))
...
(add-hook 'text-translator-window-mode-hook
          '(lambda ()
             (define-key text-translator-window-mode-map [(meta \8)] 'kill-buffer-and-window)
             ))
...
(add-hook 'comint-mode-hook
          '(lambda ()
             ...
             (define-key comint-mode-map [(control n)] 'shell-next-line)
             (define-key comint-mode-map [(control p)] 'shell-previous-line)
             ;; (define-key comint-mode-map [(control meta n)] 'next-line)
             (define-key comint-mode-map [(control meta p)] 'previous-line)
             ))

(add-hook 'compilation-mode-hook
          '(lambda ()
             (define-key compilation-mode-map [(f2)] 'kill-buffer-and-window)
...
(add-hook 'compilation-shell-minor-mode-hook
          '(lambda ()
             (define-key compilation-shell-minor-mode-map [(meta \8)] 'delete-window)
             (define-key compilation-shell-minor-mode-map [(meta n)] 'window-move-up)
             (define-key compilation-shell-minor-mode-map [(meta p)] 'window-move-down)
             (define-key compilation-shell-minor-mode-map [(control n)] 'comint-next-input)
             (define-key compilation-shell-minor-mode-map [(control p)] 'comint-previous-input)
             ))
