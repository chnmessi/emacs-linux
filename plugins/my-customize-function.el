...
(defun shell-next-line (&optional arg)
  "Open shell next-line or next-history."
  (interactive "p")
  (if (comint-after-pmark-p)
      (comint-next-input arg)
    (next-line arg)))

(defun shell-previous-line (&optional arg)
  "Open shell previous-line or previous-history."
  (interactive "p")
  (if (comint-after-pmark-p)
...
(defun google-translate-this-word ()
  (interactive)
  (if (text-translator-translate-by-auto-selection nil)
  (progn
    (deactivate-mark)
    (sleep-for 0.5)
    (other-window 1))))
...
(defun kill-word-or-delete-region (&optional arg)
  (interactive "^p")
  (if (region-active-p)
...
(defun backward-delete-to-line-beginning ()
  "delete all character from point to line beginning."
  (interactive)
  (push-mark nil t t)
  (back-to-indentation)
...
(defun convert-to-secondary-region ()
  ...
  (if (region-active-p)
      (progn
        ...
        (mouse-secondary-save-then-kill nil)
        (delete-overlay mouse-secondary-overlay)
        (deactivate-mark))))

(defun html-mark-sexp-tag ()
  (interactive)
  (push-mark nil t t)
  (sgml-skip-tag-forward 1))

(defun html-kill-sexp-tag ()
  (interactive)
  (push-mark nil t t)
  (sgml-skip-tag-forward 1)
  (kill-region (region-beginning) (region-end)))

(defun nxml-mark-sexp-tag ()
  (interactive)
  (push-mark nil t t)
  (nxml-forward-element 1))

(defun nxml-kill-sexp-tag ()
  (interactive)
  (push-mark nil t t)
  (nxml-forward-element 1)
  (kill-region (region-beginning) (region-end)))
...
(defun tm/open-newline-and-indent()
  "Function to open and goto indented next line"
  (interactive "*")
  (end-of-line)
  (newline-and-indent))
...
(defun ruby-mode-newline-and-indent ()
  (interactive)
...
(defun next-line-newline-and-indent ()
  (local-set-key [(return)] 'tm/open-newline-and-indent)
  (local-set-key [(?\r)] 'tm/open-newline-and-indent))
...
(add-hook 'ruby-mode-hook
          (lambda ()
            ...
            (local-set-key [(control j)] 'ruby-mode-newline-and-indent)
            ))

(add-hook 'python-mode-hook
          (lambda ()
            (run-coding-hook)
            (next-line-newline-and-indent)
...
(add-hook 'html-mode-hook
          (lambda ()
            ...
            (local-set-key [(control j)] 'html-mode-newline-and-indent)
            ))
...
(defun double-newline-and-indent ()
  (interactive)
  (save-excursion
    (newline-and-indent)))
...
;; (global-set-key [(meta K)] 'my-mark-lines)
